...
from ...pytorch.fusion import MixtureOfExperts
...
class BimodalBertMoE(pl.LightningModule):
    def __init__(
        self,
        ...
        # === Mixture-of-Experts fusion using MixtureOfExperts component ===
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Encode text with BertEncoder (text expert)
        text_out = self.text_encoder(input_ids, attention_mask)  # [B, fusion_dim]
        
        # Encode tabular if available (tabular expert)
        if self.tab_encoder:
        ...
        # Mixture-of-Experts fusion using MixtureOfExperts component
        fused = self.moe_fusion(text_out, tab_out)  # [B, fusion_dim]
